...
for my $file ( readdir(DIR) ) {
  if ( $file =~ /(\S+)\.fq(\.gz)?/) {
    ...
    print $jobfh "cd $dir\n";
    printf $jobfh "normalize-by-median.py -C 20 -k 20 -N 4 -x 8e8 --savehash $strain.kh $file\n";
    printf $jobfh "filter-abund.py $strain.kh $file.keep\n";
    printf $jobfh "normalize-by-median.py -C 5 -k 20 -N 4 -x 4e8 $file.keep.abundfilt\n";
